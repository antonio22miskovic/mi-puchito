							...
							@foreach ($d->inventory as $producto)
								...
								if (isset($carrito)) {
									foreach ($carrito as $item) {
										...
										if ($item->options->sale_type == "al-menor") {
											# code...
											if ($item->id == $producto->product->id) {
								...
								@endphp
									...
									<div class="card shadow">
										...
										<img style="height: 200px; object-fit: contain" data-src="{{ url('public/storage/'.$producto->product->image) }}" class="card-img-top">
								...
								@endphp
									<div class="card shadow-sm">
										...
										<img style="height: 200px; object-fit: contain" data-src='public/storage/{{ $producto->product->image }}' class="card-img-top">
										<div class="card-body body-producto">
											...
											<p class="text-center">{{ $producto->description}}</p>
													...
													@else
														<button onclick="buttonPressed('wish')"  id="deseos-{{ $producto->product->id }}"  data-id="{{ $producto->id }}" class="btn btn-block mb-2 addToWishlist">
															<i class="fa fa-heart" data-toggle="tooltip" data-title="Agregar a favoritos" style="color: #dc3545;"></i>
															<label for="deseos-{{ $producto->product->id }}" class="text-danger">Agregar a favoritos</label>
														</button>
													@endauth
